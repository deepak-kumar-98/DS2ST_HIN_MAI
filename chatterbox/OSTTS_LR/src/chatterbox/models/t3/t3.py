...
from .lora_config import LoRAConfig, apply_lora_to_model, load_lora_weights, save_lora_weights, merge_lora_weights
...
class T3(nn.Module):
    ...
    def loss(
        ...
        *,
        ...
        # Reshape for cross_entropy: [B, S, V] -> [B*S, V] and [B, S] -> [B*S]
        B, S_text, V_text = out.text_logits.shape
        B, S_speech, V_speech = out.speech_logits.shape

        text_logits_flat = out.text_logits.view(-1, V_text)
        speech_logits_flat = out.speech_logits.view(-1, V_speech)
        masked_text_flat = masked_text.view(-1)
        masked_speech_flat = masked_speech.view(-1)

        loss_text = F.cross_entropy(text_logits_flat, masked_text_flat, ignore_index=IGNORE_ID)
        loss_speech = F.cross_entropy(speech_logits_flat, masked_speech_flat, ignore_index=IGNORE_ID)

        return loss_text, loss_speech

    def apply_lora(self, lora_config: Union[LoRAConfig, dict] = None, language: str = None, rank: int = 16):
        """
        Apply LoRA adapters to the Llama backbone for fine-tuning.

        Args:
            lora_config: LoRAConfig instance or dict with config parameters
            language: Language code (e.g., "bho", "mai") - creates default config
            rank: LoRA rank if using language parameter
        ...
        if lora_config is None:
            ...
            lora_config = get_lora_config(language=language, rank=rank)
        ...
        # Required for AlignmentStreamAnalyzer during inference
    ...
    def load_lora(self, load_path: str):
        ...
        Args:
            load_path: Directory containing saved adapter weights
    ...
    def merge_lora(self):
        ...
        After merging, LoRA adapters are removed.

        Returns:
            self (for method chaining)
        """
        if not getattr(self, '_lora_applied', False):
            raise RuntimeError("LoRA not applied. Nothing to merge.")
    ...
    def set_lora_trainable(self, trainable: bool = True):
        """
        Set whether LoRA parameters are trainable.

        Args:
            trainable: If True, only LoRA params are trainable. If False, freeze all.
        """
        if not getattr(self, '_lora_applied', False):
            logger.warning("LoRA not applied. This will affect all parameters.")
