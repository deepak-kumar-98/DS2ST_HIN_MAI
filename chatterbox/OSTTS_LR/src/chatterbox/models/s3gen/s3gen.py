...
class S3Token2Wav(S3Token2Mel):
    ...
    def __init__(self):
        ...
        # silence out initial audio and fade in to reduce BOS token artifacts/"spillover"
        # Each speech token = ~40ms of audio, BOS generates 1-2 tokens worth of artifacts
        n_silence = S3GEN_SR // 10  # 100ms of silence to remove BOS artifacts
        n_fade = S3GEN_SR // 50  # 20ms fade-in
        trim_fade = torch.zeros(n_silence + n_fade)
        trim_fade[n_silence:] = (torch.cos(torch.linspace(torch.pi, 0, n_fade)) + 1) / 2
