...
def bhojpuri_normalize(text: str) -> str:
    ...
    # NFKC normalization for Devanagari
    text = unicodedata.normalize('NFC', text)

    # Bhojpuri-specific character mappings
    ...
    for old, new in nukta_mappings.items():
        text = text.replace(old, new)
...
def maithili_normalize(text: str) -> str:
    ...
    Handles Devanagari script normalization specific to Maithili.
    """
    import unicodedata
    ...
    # Maithili-specific character mappings
    replacements = {
        'ऍ': 'ए',  # Candra E to E
        'ऑ': 'ओ',  # Candra O to O
        '\u0951': '',  # Remove Udatta
    ...
    # Maithili uses some unique conjuncts - normalize them
    nukta_mappings = {
        'क़': 'क',
        'ख़': 'ख',
        'ग़': 'ग',
...
def bengali_to_devanagari(text: str) -> str:
    """
    Transliterate Bengali script to Devanagari script.
    Bengali and Devanagari are phonetically similar, so this mapping preserves pronunciation.

    Bengali Unicode range: U+0980 to U+09FF
    Devanagari Unicode range: U+0900 to U+097F
    The offset is 0x80 (128)
    """
    # Bengali to Devanagari character mapping
    # Based on Unicode block correspondence with adjustments for Bengali-specific chars
    bengali_to_devanagari_map = {
        # Vowels
    ...
    result = []
    for char in text:
        if char in bengali_to_devanagari_map:
            result.append(bengali_to_devanagari_map[char])
        else:
            result.append(char)
...
def bengali_normalize(text: str) -> str:
    ...
    Converts Bengali script to Devanagari since the tokenizer doesn't have Bengali characters.
    """
    import unicodedata
    ...
    # Convert Bengali script to Devanagari
    # This is necessary because the base tokenizer only has Devanagari characters
    text = bengali_to_devanagari(text)

    # Apply Devanagari normalization (reuse bhojpuri logic since it's now Devanagari)
    text = unicodedata.normalize('NFC', text)
...
class MTLTokenizer:
    ...
    def encode(self, txt: str, language_id: str = None, lowercase: bool = True, nfkd_normalize: bool = True):
        ...
        if language_id:
            # Bhojpuri, Maithili, and Bengali use Hindi token [hi] since they are Indic languages
            # and [bho]/[mai]/[bn] tokens are not in the base vocabulary
