#!/usr/bin/env python3
"""
Prepare Bengali data from IISc SYSPIN format for LoRA training.
...
def prepare_bengali_subset(
    ...
    """
    Convert IISc Bengali data to Chatterbox manifest format with a small subset.
...
if __name__ == "__main__":
    # Paths - UPDATE THESE TO YOUR BENGALI DATA LOCATION
    # Expected format: IISc SYSPIN data structure
    IISC_DATA_DIR = "/Users/ntiwari/IITP/chatterbox/data/IISc_SYSPIN_Data/IISc_SYSPINProject_Bengali_Female_Spk001_HC"
    JSON_PATH = f"{IISC_DATA_DIR}/IISc_SYSPINProject_Bengali_Female_Spk001_HC_Transcripts.json"
    WAV_DIR = f"{IISC_DATA_DIR}/wav"
    OUTPUT_DIR = "/Users/ntiwari/IITP/attempt3/chatterbox/data/bengali"

    # Check if data exists
    if not Path(JSON_PATH).exists():
        print(f"Bengali data not found at: {JSON_PATH}")
        print("\nPlease update the paths in this script to point to your Bengali data.")
        print("Expected format: IISc SYSPIN data structure with:")
        print("  - *_Transcripts.json file")
        print("  - wav/ directory with audio files")
        exit(1)

    # Prepare small subset for quick training (500 samples)
    prepare_bengali_subset(
        json_path=JSON_PATH,
        wav_dir=WAV_DIR,
