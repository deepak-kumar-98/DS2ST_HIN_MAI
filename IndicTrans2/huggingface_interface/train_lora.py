...
python3 train_lora.py \
    ...
    --data_dir /home/aditya/extraxtor_LLM/IndicTrans2/indic-indic-exp \
    --output_dir /mnt/storage/aditya/IndicTrans2/output_full_ft \
    --src_lang_list hin_Deva \
    --tgt_lang_list mai_Deva \
    --batch_size 8 \
    --grad_accum_steps 8 \
    --learning_rate 3e-5 \
    --max_steps 50000 \
...
# ❌ REMOVED: from peft import LoraConfig, get_peft_model
...
def get_arg_parse():
    ...
    parser.add_argument("--learning_rate", type=float, default=3e-5)  # ✏️ CHANGED: 5e-4 → 3e-5 (appropriate for full FT)
    ...
    # ❌ REMOVED: --lora_target_modules, --lora_dropout, --lora_r, --lora_alpha
    parser.add_argument(
        "--report_to",
        type=str,
        default="none",
...
def main(args):
    ...
    # ❌ REMOVED: LoraConfig(...) block
    # ❌ REMOVED: model = get_peft_model(model, lora_config)
    # ❌ REMOVED: model.print_trainable_parameters()

    model.set_label_smoothing(args.label_smoothing)

    # ✅ ADDED: Explicitly unfreeze all parameters for full finetuning
    for param in model.parameters():
        param.requires_grad = True

    # ✅ ADDED: Verify 100% params are trainable
    ...
    print(f" | > Starting full finetuning ...")
    ...
    # ✅ saves the full model weights (not just a LoRA adapter)
    model.save_pretrained(args.output_dir)
    tokenizer.save_pretrained(args.output_dir)  # ✅ ADDED: also save tokenizer alongside model
